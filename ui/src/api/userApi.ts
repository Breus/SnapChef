...
export const updatePassword = async (userId: number, currentPassword: string, newPassword: string, confirmPassword: string): Promise<void> => {
    try {
        return await put(`/users/${userId}/password`, { currentPassword, newPassword, confirmPassword }, {auth: "accessToken"});
    } catch (error) {
        if(error instanceof HttpError) {
            throw new Error(error.data.detail);
        }
