...
import { getEmail, updateEmail, updatePassword } from "../api/userApi";
...
const fetchUserEmail = async () => {
    if (userId.value !== null) {
        try {
            const email: string = await getEmail(userId.value);
            emailData.value.currentEmail = email;
        } catch (err) {
...
const submitPasswordUpdate = async () => {
    ...
    try {
        await updatePassword(userId.value, passwordData.value.currentPassword, passwordData.value.newPassword, passwordData.value.confirmPassword);
